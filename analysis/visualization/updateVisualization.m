function handles = updateVisualization(handles, xTrue, xKF, xEKF, xUKF, P_KF, P_EKF, P_UKF, trajTrue, trajKF, trajEKF, trajUKF, iteration, params, heatMap_KF, heatMap_EKF, heatMap_UKF)
...
if gpsAvailable
    ...
    gpsColor = [0.5 0.5 0.5];
    gpsText = 'GPS: OFF';
end

text(0.02, 0.98, sprintf('Iter: %d/%d', iteration, params.maxIterations), 'Units', 'normalized', 'VerticalAlignment', 'top', 'FontSize', 9, 'BackgroundColor', 'white', 'EdgeColor', 'black');
...
xlabel('x [m]');
ylabel('y [m]');
title('KF: Path Certainty', 'FontWeight', 'bold', 'Color', params.colorKF);
grid on;
...
xlabel('x [m]');
ylabel('y [m]');
title('EKF: Path Certainty', 'FontWeight', 'bold', 'Color', params.colorEKF);
grid on;
...
xlabel('x [m]');
ylabel('y [m]');
title('UKF: Path Certainty', 'FontWeight', 'bold', 'Color', params.colorUKF);
grid on;
