function handles = updateVisualizationWithObstacles(handles, xTrue, xKF, xEKF, xUKF, P_KF, P_EKF, P_UKF, ...
                                                     trajTrue, trajKF, trajEKF, trajUKF, iteration, params)
% Updates visualization including obstacles
%
% INPUTS:
%   handles   - Structure with graphics handles (includes obstacles field)
%   xTrue     - True state
...
%% Main plot (top view with obstacles)
nexttile(handles.tiles, 1);
cla; hold on;
grid on;
xlabel('$x$ [m]', 'Interpreter', 'latex');
ylabel('$y$ [m]', 'Interpreter', 'latex');
title('Robot Trajectories (Top View)', 'Interpreter', 'latex');

% Plot obstacles FIRST (so they're in background)
plotObstaclesSimple(handles.obstacles);

% Plot trajectories
plot(trajTrue(1, :), trajTrue(2, :), '-', 'Color', params.colorTrue, 'LineWidth', 2);
plot(trajKF(1, :), trajKF(2, :), '--', 'Color', params.colorKF, 'LineWidth', 1.5);
plot(trajEKF(1, :), trajEKF(2, :), '--', 'Color', params.colorEKF, 'LineWidth', 1.5);
plot(trajUKF(1, :), trajUKF(2, :), '--', 'Color', params.colorUKF, 'LineWidth', 1.5);
...
legend('True', 'KF', 'EKF', 'UKF', 'Location', 'best');

%% Position error plots (same as before)
...
ylabel('$x$ Error [m]', 'Interpreter', 'latex');
...
xlabel('Iteration', 'Interpreter', 'latex');
ylabel('$y$ Error [m]', 'Interpreter', 'latex');
...
legend('KF', 'EKF', 'UKF', 'Location', 'best');
